...
DP_raw = read.csv(file = "data/raw/EXP-NEW/DP.csv", header = T)
DP = DP_raw %>% filter(Screen == "Testing")
DP = DP %>% rename(CorrectAnswer = Spreadsheet..CorrectAnswer)
DP_ref = read.csv(file = "data/raw/EXP-NEW/Dual-Plural-Ref.csv", header = T)
...
SD_raw = read.csv(file = "data/raw/EXP-NEW/SD.csv", header = T)
SD = SD_raw %>% filter(Screen == "Testing")
SD = SD %>% rename(CorrectAnswer = Spreadsheet..CorrectAnswer)
SD_ref = read.csv(file = "data/raw/EXP-NEW/Singular-Dual-Ref.csv", header = T)

SD <- merge(x=SD,y=SD_ref,by="CorrectAnswer")
#sd <- SD %>% select(Participant.Public.ID,Participant.Private.ID,randomiser.3q16,Reaction.Time,Correct,Condition,Critical)


SP_raw = read.csv(file = "data/raw/EXP-NEW/SP.csv", header = T)
SP = SP_raw %>% filter(Screen == "Testing")
SP = SP %>% rename(CorrectAnswer = Spreadsheet..CorrectAnswer)
SP_ref = read.csv(file = "data/raw/EXP-NEW/Singular-Plural-Ref.csv", header = T)

SP <- merge(x=SP,y=SP_ref,by="CorrectAnswer")
...
data <- raw_data %>% select(Participant,Condition, Response, Correct)
